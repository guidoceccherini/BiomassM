...
# load drivers
drivers <- rast("Data/italy/disturbance_agent_1985_2023_italy.tif")
# # # # Extract layers for 2015-2020 (layers 31-36)
# drivers_2015_2020 <- drivers[[31:36]]
# # Create a mask from the binary raster (cells where disturbance = 1)
# mask <- disturbance_binary == 1
# 
# # OPTION A: Get the maximum agent value across the 2015-2020 period
# agents_max <- app(drivers_2015_2020, max, na.rm = TRUE)
# 
...
#             "Data/disturbance_agents_2015_2020_filtered.tif", 
#             overwrite = TRUE)


result_raster <- rast( "Data/disturbance_agents_2015_2020_filtered.tif")

# # OPTION B (Alternative): Get the first non-zero agent encountered in the time series
# agents_first_nonzero <- app(drivers_2015_2020, function(x) {
#   non_zero <- x[x != 0 & !is.na(x)]
#   if(length(non_zero) > 0) return(non_zero[1])
#   else return(NA)
# })
# 
# # Apply the binary mask
# result_raster_alt <- agents_first_nonzero
# result_raster_alt[!mask] <- NA
...
DeltaBiomass <- Biomass2010_aligned - Biomass2020_aligned
...
Biomass2010_pos <- ifel(DeltaBiomass > 0, DeltaBiomass, NA)
...
biomass_forest_stack <- c(biomass_disturbed[[1]], EEA_forest_type_aligned, result_raster)
names(biomass_forest_stack) <- c("biomass", "forest_type","driver")
...
biomass_dt[, driver_type_label := fcase(
  driver_type == 1,   "Wind or Bark beetle ",
  driver_type == 2, "Fire",
  driver_type == 3, "Harvest",
  default = NA_character_
)]
...
# Save to disk
fwrite(biomass_dt, "biomass_complete_data_deltaABGDriversDelta.csv")
...
p1 <- ggplot(biomass_df, aes(x = biomass, fill = forest_type_label)) +
  ...
  facet_grid(driver_type_label~.)+
  labs(title = "Biomass Loss (2020 - 2010) Distribution in Disturbed Pixels",
...
biomass_forest_bg_stack <- c(biomass_disturbed[[1]], 
                             ...
                             BG_numeric_raster,
                             result_raster)
names(biomass_forest_bg_stack) <- c("biomass", "forest_type", "biogeoregion", "driver")
...
fwrite(biomass_dt, "biomass_complete_data_with_bg_deltaABGDriversDelta.csv")
...
p1 <- ggplot(biomass_df, aes(x = biomass, fill = forest_type_label)) +
  ...
  facet_wrap(driver_type_label~biogeoregion_label, scales = "free") +
  labs(
    title = "Distribution of Biomass Loss (2020 - 2010) in Disturbed Pixels by Bio-geographical Region",
...
print(p1)
ggsave("Figures/biomass_by_biogeoregion_density_AGBLOSSDriversDelta.png", p1, width = 16, height = 12, dpi = 300)
...
p2 <- ggplot(biomass_df, aes(x = forest_type_label, y = biomass, 
  ...
  facet_wrap(driver_type_label~biogeoregion_label, scales = "free_y") +
  labs(
    title = "Biomass Loss Distribution in Disturbed Pixels by Bio-geographical Region",
    x = "Forest Type",
...
print(p2)
ggsave("Figures/biomass_by_biogeoregion_violinDriversDelta.png", p2, width = 16, height = 12, dpi = 300)
...
p3 <- ggplot(biomass_df, aes(x = forest_type_label, y = biomass, 
  ...
  facet_wrap(driver_type_label~biogeoregion_label, scales = "free_y") +
  labs(
    title = "Biomass Loss (2020 - 2010) in Disturbed Pixels by Bio-geographical Region",
    x = "Forest Type",
    y = "Above Ground Biomass (Mg/ha)"
  ) +
  theme_minimal() +
...
print(p3)
ggsave("Figures/biomass_by_biogeoregion_boxplot_AGBLOSSDriversDelta.png", p3, width = 16, height = 12, dpi = 300)
...
# Save summary to CSV
fwrite(summary_stats, "Data/biomass_summary_by_biogeoregion_forest_AGBLOSSDrivers.csv")
...
summary_for_plot <- biomass_dt[, .(
  ...
  sd_biomass = sd(biomass, na.rm = TRUE),
  n = .N
), by = .(biogeoregion_label, forest_type_label,driver_type_label)]
...
p4 <- ggplot(summary_for_plot, aes(x = biogeoregion_label, y = mean_biomass, 
  ...
  labs(
    title = "Mean Biomass Loss (2020 - 2010) in Disturbed Pixels",
...
print(p4)
ggsave("Figures/biomass_summary_by_biogeoregion_AGBLOSSDriversDelta.png", p4, width = 12, height = 6, dpi = 300)
...
# 
# print(p5)
# ggsave("Figures/pixel_counts_by_biogeoregion_AGBLOSSDriversDelta.png", p5, width = 12, height = 6, dpi = 300)
